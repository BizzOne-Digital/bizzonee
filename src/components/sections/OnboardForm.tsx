...
export default function OnboardForm() {
    ...
    return (
      <div className="mx-auto mt-10 max-w-3xl rounded-3xl glass-strong p-10 text-center">
        ...
        <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-white/60">
          Thanks {f.name.split(" ")[0] || "there"} — your onboarding details have been sent to our team. We&apos;ll reach out within 24–48 hours.
        </p>
        {taskUrl && (
          <a href={taskUrl} target="_blank" rel="noreferrer" className="mt-6 inline-flex items-center gap-2 rounded-full neon-border px-6 py-3 text-sm font-bold text-white transition-all hover:shadow-glow-purple">
            View in ClickUp
          </a>
        )}
      </div>
  ...
  return (
    <div className="mx-auto mt-10 max-w-3xl rounded-3xl glass-strong p-6 sm:p-8">
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <div><label className={labelCls}>Package</label><Select value={f.pkg} onChange={(v) => set("pkg", v)} placeholder="Select package" options={PKG_OPTIONS} /></div>
        <div><label className={labelCls}>Main goal of the website</label><Select value={f.goal} onChange={(v) => set("goal", v)} placeholder="Select goal" options={GOALS} /></div>
      </div>
