...
const PROJECTS: Project[] = [
  ...
  { name: "AEM Quality ISO", category: "Certification", description: "Clean, credible site for ISO certification consulting.", url: "https://www.aemqualityiso.com" },
...
function MiniFrame({ url, name, mounted }: { url: string; name: string; mounted: boolean }) {
      ...
      <div ref={ref} className="relative aspect-[16/10] w-full overflow-hidden bg-[#05030a]">
        {mounted ? (
          ...
          <div className="absolute inset-0 grid place-items-center bg-gradient-to-br from-[#1a0b2e] to-[#05030a]">
            <div className="animate-pulse text-xs font-semibold text-white/40">{name}</div>
          </div>
        )}
      </div>
    </div>
            ...
            <div className="relative" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
              <AnimatePresence mode="wait">
                <motion.div key={active} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.35 }}>
                  <MiniFrame url={cur.url} name={cur.name} mounted={true} />
                </motion.div>
              </AnimatePresence>
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <a href={cur.url} target="_blank" rel="noreferrer" className="grid h-8 w-8 place-items-center rounded-full glass text-white/60 transition hover:text-brand-mint">
            ...
            {PACKAGES.map((p, i) => (
              <Reveal key={p.name} delay={i * 0.07}>
                <div className={`relative flex h-full flex-col rounded-3xl p-6 transition-all duration-300 hover:-translate-y-2 ${p.popular ? "neon-border shadow-glow-purple" : "glass hover:shadow-glow-purple"}`}>
                  ...
                  <button onClick={scrollToOnboard} className={`mt-6 inline-flex items-center justify-center gap-2 rounded-full px-5 py-3 text-sm font-bold transition-all ${p.popular ? "bg-brand-mint text-ink shadow-glow-mint hover:-translate-y-0.5" : "neon-border text-white hover:shadow-glow-purple"}`}>
